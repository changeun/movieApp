## Props
- 부모클래스에서 선언한 변수들을 자식클래스에서 불러쓸 때 사용
- 사용방법 : {this.props.["변수"]}

## State
- 컴포넌트를 로드하는 방법
- 컴포넌트 안에 State가 바뀔 때마다 render 작동
- 사용방법 : {this.state.["변수"]}
- 참고 : this.state.["변수"] = "value" 형식을 사용하면 절대 안됨. 왜냐하면 state는 render를 발생시키기 때문. 그래서 setState를 써줌.

## setState
- 변경된 State를 업데이트할 때 사용
- this.setState({ 새로운 State })

## Ajax on React
- 참고 사이트 : yts api (https://yts.am/api)
- 사용방법 : 
  > fetch(url주소)
  > promise (스케쥴 작업 용이, 시나리오 잡는 방법을 만들어줌)
